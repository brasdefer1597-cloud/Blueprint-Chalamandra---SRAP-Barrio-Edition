...
// Optimization: Cache the last rendered state to prevent unnecessary DOM writes (dirty checking)
// Reduces DOM reflows by only updating textContent/className when values actually change
...
function updateScores() {
  // Only write to DOM if the value has changed
  ...
  if (metricFlow) {
    ...
    const newClass = flowControl > 1.5 ? "text-lime-400" : "text-yellow-400";
    if (lastRenderedState.flowControlClass !== newClass) {
      metricFlow.className = newClass;
      lastRenderedState.flowControlClass = newClass;
    }
  }
}
