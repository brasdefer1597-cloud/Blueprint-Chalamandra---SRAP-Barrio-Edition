...
// ⚡ Bolt Optimization: Cache state to prevent redundant DOM updates
...
function updateScores() {
  if (gameState.insightPoints !== lastRenderedState.insightPoints) {
  ...
  if (
    metricDisaster &&
    gameState.epicDisasterLevel !== lastRenderedState.epicDisasterLevel
  ) {
  ...
  if (metricFlow) {
    if (flowControl !== lastRenderedState.flowControl) {
      metricFlow.textContent = flowControl;
      lastRenderedState.flowControl = flowControl;
    }
    const newClassName =
      flowControl > 1.5 ? "text-lime-400" : "text-yellow-400";
    if (newClassName !== lastRenderedState.flowClassName) {
      metricFlow.className = newClassName;
      lastRenderedState.flowClassName = newClassName;
    }
  }
}
