...
function enhanceAccessibility() {
  ...
  // Custom interactive elements (need role and tabindex)
  document.querySelectorAll(selectorString).forEach((element) => {
    element.setAttribute("role", "button");
    element.setAttribute("tabindex", "0");
  });

  // All interactive elements including standard buttons (for focus styles)
  const allInteractiveSelectors = [
    ...interactiveSelectors,
    ".cta-button",
    ".nav-level-button",
  ].join(", ");
  document.querySelectorAll(allInteractiveSelectors).forEach((element) => {
