...
function updateScores() {
  // Optimization: Dirty checking to prevent unnecessary DOM writes
  if (_lastRenderedScores.insightPoints !== gameState.insightPoints) {
    insightCounter.textContent = gameState.insightPoints;
    _lastRenderedScores.insightPoints = gameState.insightPoints;
  }
  ...
  if (
    metricDisaster &&
    _lastRenderedScores.epicDisasterLevel !== gameState.epicDisasterLevel
  ) {
  ...
  if (metricFlow) {
    if (_lastRenderedScores.flowControl !== flowControl) {
      metricFlow.textContent = flowControl;
      _lastRenderedScores.flowControl = flowControl;
    }

    const flowClassName =
      flowControl > 1.5 ? "text-lime-400" : "text-yellow-400";
    if (_lastRenderedScores.flowClassName !== flowClassName) {
      metricFlow.className = flowClassName;
      _lastRenderedScores.flowClassName = flowClassName;
    }
  }
}
