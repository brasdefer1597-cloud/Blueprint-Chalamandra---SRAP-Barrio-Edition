...
function sanitizeHTML(html) {
  ...
  allElements.forEach((el) => {
    for (let i = el.attributes.length - 1; i >= 0; i--) {
      ...
      // SECURITY: Strip control characters (\x00-\x20) and Unicode replacement chars (\uFFFD)
      // to prevent XSS payloads that use them to bypass naive string matching (e.g. java\x00script:)
