...
// Optimization: JS-level state cache for dirty checking to minimize redundant DOM manipulations
...
function updateScores() {
  // Dirty checking: only update DOM if value changed
  ...
  const flowClassName = flowControl > 1.5 ? "text-lime-400" : "text-yellow-400";
  if (lastRenderedState.flowClassName !== flowClassName) {
    metricFlow.className = flowClassName;
    lastRenderedState.flowClassName = flowClassName;
  }
}
