...
def run_test():
    ...
    try:
        with sync_playwright() as p:
            ...
            initial_class = step.get_attribute("class")
            print(f"Initial class: {initial_class}")

            if "srap-active" in initial_class:
                print("FAIL: Step should not be active initially.")
                browser.close()
                sys.exit(1)
            ...
            # 3. Post-Click Check
            new_class = step.get_attribute("class")
            print(f"New class: {new_class}")

            if "srap-active" not in new_class:
                print("FAIL: Step did not become active.")
                browser.close()
                sys.exit(1)

            # 4. Metrics Check
            counter = page.locator("#insight-counter")
            insight_points = counter.text_content()
            print(f"Insight Points: {insight_points}")

            if insight_points == "0":
                print("FAIL: Insight points did not increase.")
                browser.close()
                sys.exit(1)
